...
import { BartenderMusicMatterMoreBar } from "app/model.client/BartenderMusicMatterMoreBar";
import { ResResult } from "app/model.client/ResResult";
import { BarRating } from "app/model.client/BarRating";

@Component({
  selector: 'bartenderMusicBar',
  templateUrl: './bartenderMusicBar.component.html'
})
export class BartenderMusicBar implements OnInit {
  ...
  constructor(         
    ...
    userFullDetail:User;
    Bars:BartenderMusicMatterMoreBar[] = [];
    ...
    getNameandUser(): void {
        this.route.params.subscribe((params: Params) => {
            let user = params['userdetailbarmus'];
            ...
            this.webService.getUser(this.user).subscribe(res => {
      			...
      			this.webService.getBartenderMusicBar(this.userFullDetail).subscribe(res => {
            this.Bars = res;
